...
class AsyncConfirmationExecutor:
    CONF_URL = 'https://steamcommunity.com/mobileconf'

    def __init__(self, identity_secret: str, my_steam_id: str, session: AsyncSession) -> None:
    ...
    async def _send_confirmation(self, confirmation: Confirmation) -> dict:
        ...
        resp = await self._session.get(f'{self.CONF_URL}/ajaxop', params=params, headers=headers)
        return resp.json()
    ...
    async def _fetch_confirmations_page(self):
        tag = Tag.CONF.value
        params = self._create_confirmation_params(tag)
        headers = {'X-Requested-With': 'com.valvesoftware.android.steam.community'}
        response = await self._session.get(f'{self.CONF_URL}/getlist', params=params, headers=headers)
    ...
    async def _fetch_confirmation_details_page(self, confirmation: Confirmation) -> str:
        ...
        params = self._create_confirmation_params(tag)
        response = await self._session.get(f'{self.CONF_URL}/details/{confirmation.data_confid}', params=params)
